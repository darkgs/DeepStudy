...
#			targets = pack_padded_sequence(captions, lengths, batch_first=True)[0]
	...
	def test(self):
		...
		with torch.no_grad():
			...
			data = next(iter(self.data_loader['val']))
			...
			# generate caption
			target_idx = random.randrange(0, batch_size)
			image_data = images[target_idx].cpu().numpy()
			sampled_ids = sampled_ids[target_idx].cpu().numpy()
			...
			output_caption = ' '.join(generated_caption)
		return image_data, output_caption
